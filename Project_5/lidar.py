...
def _in_wf_left_zone(angle):
    return WF_LEFT_MIN <= angle <= WF_LEFT_MAX
...
class LidarMonitor:
    ...
    @property
    def left_dist(self):
        """Minimum distance (mm) in left zone (55-135°), or None if no valid reading."""
        with self._lock:
            ...
            try:
                ...
                for scan in self._lidar.iter_scans():
                    ...
                    for (quality, angle, distance) in scan:
                        ...
                        if _in_wf_left_zone(angle):
                            wf_left_readings.append(distance)
                    ...
                    new_left_dist  = min(wf_left_readings)  if wf_left_readings  else None
