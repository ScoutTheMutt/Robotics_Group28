...
class WallFollower:
    ...
    def _decide(self, front, right, fr):
        ...
        # Case 2: too close (also check front-right so we react early to corners)
        if right < WALL_LOWER_MM:
            return 'STEER_AWAY'
        if fr is not None and fr < WALL_LOWER_MM:
            return 'STEER_AWAY'
    ...
    def _execute(self, state):
        if state == 'FORWARD':
            ...
            self._robot.turnLeft(TURN_SPEED)

        elif state == 'SEARCH':
            # Gently turn right to sweep for the wall
            self._robot.turnRight(TURN_SPEED * 0.6)
